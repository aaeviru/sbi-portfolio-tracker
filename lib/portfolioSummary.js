...
function applyBuy(summary, tx, amount) {
  ...
  if (summary.assetSubType == 'MMF') {
    var importedUnitPrice = isNumber(tx.unitPrice) && tx.unitPrice > 0
      ? tx.unitPrice
...
function applyAssetPrice(summary, asset) {
  ...
  if (summary.latestPrice != null) {
    ...
    summary.unrealizedPlPercent = summary.remainingCost > 0
      ...
      : null;
    summary.totalPl = roundMoney(summary.unrealizedPl + summary.fifoRealizedPl);
  }
}
...
function buildPortfolioSummary(transactions, assetsBySymbol) {
  ...
  sorted.forEach(function (tx) {
    ...
    var fxRate = isNumber(asset.latestFxRate) ? asset.latestFxRate : null;
