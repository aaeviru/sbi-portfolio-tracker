...
function summarizeCoverage(asset, sourceRanges, rows, intervals, nowText) {
  nowText = nowText || new Date().toISOString();
  ...
  var dueFailures = failed.filter(function (interval) {
    return interval.error || !interval.retryAfter || interval.retryAfter <= nowText;
  });
  ...
  return {
    ...
    status: dueFailures.length
      ? 'FAILED'
      : failed.length
        ? 'RETRY_SCHEDULED'
        : windows.length ? 'NEEDS_UPDATE' : 'UP_TO_DATE'
  };
}
